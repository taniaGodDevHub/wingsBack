!function(o) {
    ...
    function e() {
        ...
        t && t.addEventListener("click", () => {
                ...
                e = "Nice, you triggered this alert message!",
                    t = "success",
                    (n = document.createElement("div")).innerHTML = [`<div class="alert alert-${t} alert-dismissible" role="alert">`, `   <div>${e}</div>`, '   <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>', "</div>"].join(""),
                    a.append(n)
            }
        )
        document.getElementById("main-css").href.includes("rtl.min.css") && (document.getElementsByTagName("html")[0].dir = "rtl")
    }
    function t() {
        if (!$(".side-nav").length) return;

        // Очистим все старые active/menuitem-active классы
        $(".side-nav a").removeClass("active");
        $(".side-nav li").removeClass("menuitem-active");
        $(".side-nav .collapse").removeClass("show");
        $(".side-nav a[data-bs-toggle='collapse']").addClass("collapsed");
        ...
        $(".side-nav a").each(function () {
            ...
            if (linkUrl === currentUrl) {
                ...
                $(this).closest("li").addClass("menuitem-active");
                $(this).parents(".collapse").each(function () {
                    $(this).addClass("show");
                    $(this).prev("[data-bs-toggle='collapse']").removeClass("collapsed");
                    $(this).closest("li").addClass("menuitem-active");
                });
            }
        });
        ...
        setTimeout(function () {
            let activeLink = document.querySelector("li.menuitem-active .active");
            if (activeLink) {
                let container = document.querySelector(".leftside-menu .simplebar-content-wrapper");
                if (container) {
                    let offset = activeLink.offsetTop - 300;
                    if (offset > 100) {
    ...
    o(".navbar-nav").length && (o(".navbar-nav li a").each(function() {
        ...
        o("#top-search").on("input", function(){
            var q = this.value.trim();
            var $dd = o("#search-dropdown");
            if (!q) {
                $dd.find('#search-total').text('0');
                $dd.find('#search-projects, #search-tasks').empty();
                $dd.find('#search-projects-header, #search-tasks-header').hide();
                return;
            }
            var $root = o('#app-search-root');
            var explicitUrl = $root.data('apiUrl');
            var base = (window.baseConfig && window.baseConfig.indexUrl) ? window.baseConfig.indexUrl : '/index.php';
            var url = explicitUrl || (base + (base.indexOf('?') === -1 ? '?r=api/search' : '&r=api/search'));
            o.getJSON(url, { q: q }, function(resp){
                if (!resp || !resp.success) { return; }
                $dd.addClass('d-block');
                $dd.find('#search-total').text(resp.count);
                var $p = $dd.find('#search-projects');
                var $t = $dd.find('#search-tasks');
                $p.empty();
                $t.empty();
                if (resp.projects && resp.projects.length) {
                    $dd.find('#search-projects-header').show();
                    resp.projects.forEach(function(pr){
                        var name = pr.name || '';
                        var desc = pr.description || '';
                        var href = baseConfig.indexUrl + '?r=task/default/index&project_id=' + pr.id;
                        var html = '<a href="'+href+'" class="dropdown-item notify-item">'
                            + '<i class="uil-folder font-16 me-1"></i>'
                            + '<span>#'+pr.id+' '+o('<div>').text(name).html()+'</span>'
                            + '</a>';
                        $p.append(html);
                    });
                } else {
                    $dd.find('#search-projects-header').hide();
                }
                if (resp.tasks && resp.tasks.length) {
                    $dd.find('#search-tasks-header').show();
                    resp.tasks.forEach(function(ts){
                        var name = ts.name || '';
                        var href = baseConfig.indexUrl + '?r=task/default/index&project_id=' + ts.project_id + '&selected_task_id=' + ts.id;
                        var html = '<a href="'+href+'" class="dropdown-item notify-item">'
                            + '<i class="uil-check-square font-16 me-1"></i>'
                            + '<span>#'+ts.id+' '+o('<div>').text(name).html()+'</span>'
                            + '</a>';
                        $t.append(html);
                    });
                } else {
                    $dd.find('#search-tasks-header').hide();
                }
            });
        }),
...
class ThemeCustomizer {
    ...
    setSwitchFromConfig() {
        ...
        var e, t, n, a, o, i, s, r, c = this.config;
        c && (e = document.querySelector("input[type=radio][name=data-layout][value=" + c.nav + "]"),
            t = document.querySelector("input[type=radio][name=data-bs-theme][value=" + c.theme + "]"),
            ...
            o = document.querySelector("input[type=radio][name=data-menu-color][value=" + c.menu.color + "]"),
            i = document.querySelector("input[type=radio][name=data-sidenav-size][value=" + c.sidenav.size + "]"),
            s = document.querySelector("input[type=radio][name=data-layout-position][value=" + c.layout.position + "]"),
