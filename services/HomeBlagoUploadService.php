...
final class HomeBlagoUploadService
{
    ...
    {
        return Yii::getAlias('@uploads/blago');
    }

    public function upload(HomeBlago $blago, UploadedFile $file): ?string
    {
        if ($file->hasError) {
        ...
        $filename = sprintf('blago_%s.%s', bin2hex(random_bytes(8)), $extension);
        ...
        $this->removeLocalFileIfOwned($blago->image_url);
        $blago->image_url = 'uploads/blago/' . $filename;
