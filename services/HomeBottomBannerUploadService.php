...
final class HomeBottomBannerUploadService
{
    ...
    {
        return Yii::getAlias('@uploads/bottom-banner');
    }

    public function upload(HomeBottomBanner $banner, UploadedFile $file): ?string
    {
        if ($file->hasError) {
        ...
        $filename = sprintf('bottom_%s.%s', bin2hex(random_bytes(8)), $extension);
        ...
        $banner->image_url = 'uploads/bottom-banner/' . $filename;
